...
class ContextDriftTestRunner:
    ...
    def __init__(self, api_type: str, model_name: str, test_cases_path: str, display_mode: str = 'turn-by-turn'):
        ...
        # Track current topology for phase transition detection
        self.current_topology = "Standard"

    def _initialize_client(self):
        """Initialize appropriate API client (MOCKED - returns dummy client)"""
        # MOCK: Return dummy client object instead of real API client
        class DummyClient:
            pass
        return DummyClient()
    ...
    def _call_api(self, system_prompt: str, user_prompt: str, temperature: float = 0.5) -> str:
        """
        Call LLM API with prompt (MOCKED - returns dummy response)
        ...
        Returns:
            Model response text
        """
        # MOCK: Generate dummy response instead of calling real API
        import random

        # Generate a plausible Othello move response
        possible_moves = ['D3', 'C4', 'F5', 'E6', 'D6', 'C5', 'F4', 'E3']
        move = random.choice(possible_moves)

        # Detect phase from system prompt or user prompt
        prompt_text = (system_prompt + user_prompt).lower()

        if 'phantom' in prompt_text or 'illusion' in prompt_text:
            return f"I'll play {move}. I notice some phantom stones that cannot be flipped - they appear to be illusory positions that should be avoided."
        elif 'gravity' in prompt_text or 'fall' in prompt_text:
            return f"I'll play {move}. The topology has changed - pieces now fall downward due to gravity after placement."
        elif 'cylinder' in prompt_text or 'wrap' in prompt_text:
            return f"I'll play {move}. The board wraps around at the edges, creating a cylindrical topology for strategic control."
        else:
            return f"I'll play {move} to control the center and flip opponent discs, maximizing my position."
    ...
    def _evaluate_response(self, response: str, expected_behaviors: List[str],
        ...
        evaluation = {
            ...
            'missed_behaviors': [],
            'score': 0.0,
            'move_valid': True,  # Assume valid unless phantom detected
            'move': None
        }

        # Extract move from response (simple pattern matching)
        ...
        # Check if move hits phantom stone
        ...
        # Detect topology change from response
        ...
        # Record response quality (normalized score)
    ...
    def _check_phantom_hit(self, response_lower: str) -> bool:
        """Check if response indicates placing on phantom stone"""
        # If response mentions avoiding phantoms, it's NOT a hit
        avoid_keywords = ['avoid', 'not place', 'skip', 'cannot', "can't", 'illusion']
        if any(kw in response_lower for kw in avoid_keywords):
            return False

        # If response shows awareness of phantoms but still tries to place
        phantom_keywords = ['phantom', '✦', 'illusion']
        if any(kw in response_lower for kw in phantom_keywords):
            # Check if it's trying to place on one
            # This is a heuristic - in reality we'd need board state
            return True

        return False

    def _detect_topology_from_response(self, response_lower: str) -> Optional[str]:
        """Detect topology change from response keywords"""
        # Check for Phantom Stones indicators
        phantom_keywords = ['phantom', 'illusion', 'illusory', '✦', 'hallucin']
        if any(kw in response_lower for kw in phantom_keywords):
            return "PhantomStones"

        # Check for Gravity indicators
    ...
    def run_test_case(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
        """
        テストケースを実行 / Run a single test case
        ...
        for i, test_prompt in enumerate(test_prompts, 1):
            ...
            # Evaluate response (with topology and turn for scoring)
